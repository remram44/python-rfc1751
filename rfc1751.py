...
class WrongWordError(Error):
    """Unknown word in RFC 1751 conversion"""
    def __init__(self, word):
        self.word = word

    def __str__(self):
        return f"Unknown word {self.word!r} in RFC 1751 conversion"


class WrongNumberOfWordsError(Error):
    """Wrong number of words in RFC 1751 conversion"""
    def __init__(self, number):
        self.number = number

    def __str__(self):
        return f"Got {self.number} words, expected 6"


class WrongNumberOfBytesError(Error):
    """Wrong number of bytes in RFC 1751 conversion"""
    def __init__(self, number):
        self.number = number

    def __str__(self):
        return f"Got {self.number} bytes, expected 8"


class ParityError(Error):
    """Parity error in RFC 1751 conversion"""


def _extract_bits(array, start, length):
    assert 0 <= length <= 11
    assert start >= 0
    assert start + length <= 66

    array = array + b'\x00\x00'
